<template>
  <Page>
    ...
    <GridLayout columns="*" rows="*, *, *, *, *, *">
      ...
      <Label class="message" :text="`errorLog: ${errorLog}`" col="0" row="2"/>
      <Label class="message" :text="`events: ${events}`" col="0" row="3"/>
      <Label class="message" :text="`trackonce: ${trackonce}`" col="0" row="4"/>
      <!-- <StackLayout col="0" row="5">
        <Label class="message" :text="`currentLocation latitude: ${currentLocation.latitude}`" />
        <Label class="message" :text="`currentLocation longitude: ${currentLocation.longitude}`" />
        <Label class="message" :text="`currentLocation accuracy: ${currentLocation.accuracy}`" />
      </StackLayout> -->
    </GridLayout>
  </Page>
</template>
